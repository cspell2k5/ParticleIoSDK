...
extension PCClient {
    ...
    ///````swift
    ///
    ///        PCClient.listClients(productIdorSlug: "cool_product_name", token: token)
    ///            .sink { completion in
    ///                switch completion {
    ...
    ///                if let success = response?.ok {
    ///                    //successful request
    ///                    if let user = response?.clients.first {
    ///                        let clientCredentials = PCCredentials(username: user.id, password: user.secret)
    ///                        //create an api user or something
    ///                    }
    ///                }
    ...
    /// - Returns: CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError>
    public static func listClients(productIdorSlug: ProductID?, token: PCAccessToken) -> CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError> {
        PCNetwork.shared.cloudRequest(.listClients(productIdorSlug: productIdorSlug, token: token), type: PCClientSeverResponse.ServerResponse.self)
    }
    ...
    ///````swift
    ///
    ///        PCClient.createClient(appName: "cool_apps", productIdorSlug: nil, type: .installed, token: token)
    ///            .sink { completion in
    ///                switch completion {
    ///                    case .failure(let error):
    ///                            //handle error
    ...
    ///                        let clientCredentials = PCCredentials(username: client.id, password: client.secret)
    ...
    /// - Parameter token: An PCAccessToken carrying the access token and associated information.
    /// - Returns: `CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError>`
    public static func createClient(appName: String, productIdorSlug: ProductID?, type: PCClient.ClientType, token: PCAccessToken)
    -> CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError> {
        PCNetwork.shared.cloudRequest(.createClient(appName: appName, productIdorSlug: productIdorSlug, type: type, token: token), type: PCClientSeverResponse.ServerResponse.self)
    }
    ...
    ///````swift
    ///
    ///        PCClient.updateClient(client: client, newName: "clever_client_name", newScope: .fullControl, productIdorSlug: nil, token: token)
    ///            .sink { completion in
    ///                switch completion {
    ///                    case .failure(let error):
    ///                            //handle error
    ...
    /// - Parameter token: An PCAccessToken carrying the access token and associated information.
    /// - Returns: `CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError>`
    public static func updateClient(client: PCClient, newName: String?, newScope: PCClient.Scope?, productIdorSlug: ProductID?, token: PCAccessToken) -> CurrentValueSubject<PCClientSeverResponse.ServerResponse?, PCError> {
        PCNetwork.shared.cloudRequest(.updateClient(client: client, newName: newName, newScope: newScope, productIdorSlug: productIdorSlug, token: token), type: PCClientSeverResponse.ServerResponse.self)
    }
    ...
    ///````swift
    ///
    ///        PCClient.deleteClient(_ client: client, productIdorSlug: nil, token: token)
    ///            .sink { completion in
    ///                switch completion {
    ///                    case .failure(let error):
    ///                            //handle error
    ...
    public static func deleteClient(_ client: PCClient, productIdorSlug: ProductID?, token: PCAccessToken) -> CurrentValueSubject<ServerResponses.NoResponse?, PCError> {
        PCNetwork.shared.cloudRequest(.deleteClient(client: client, productIdorSlug: productIdorSlug, token: token), type: ServerResponses.NoResponse.self)
    }
}
