...
/**
 ...
 let token = try await PCAccessToken.generateAccessToken(credentials: PCCredentials(username: "particle_username", password: "particle_password"))
 ...
 Note: A token can be created using a token string, but this behavior is currently discouraged in this package.  The server is currently relied on for security and should issue a new token whenever this package needs one.
 */
...
"""
    ...
    ///Public but discouraged initializer. This init may be replaced in future updates.
    ...
    ///Prefered init meant to decode a json token sent from the server.
    ...
    //FIXME: - Unsafe
    ///Public for saving SAFELY in keychain this will likely / should be replaced with safer versions in future updates.
